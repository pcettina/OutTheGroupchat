 ...
 *
 * Rate-limit tests use toBe(429) — rate limiting is now live on both routes.
 * Assertions were tightened from forward-looking toContain([...]) patterns.
 */
...
describe('GET /api/trips/[tripId]/itinerary', () => {
  ...
  it('returns 429 when rate limit is exceeded', async () => {
    ...
    vi.mocked(checkRateLimit).mockResolvedValueOnce({ success: false, limit: 10, remaining: 0, reset: Date.now() + 60000 } as Awaited<ReturnType<typeof checkRateLimit>>);
