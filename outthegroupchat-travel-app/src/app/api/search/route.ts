...
// Global search across people, meetups, and venues (people-first).
// Legacy trip-domain types (trips/activities/users) are no longer accepted
// and will return 400 via Zod validation.
export async function GET(req: Request) {
  try {
    ...
    // Search users (people) - email omitted for privacy (prevents user enumeration attacks)
    ...
    // For 'all' type, order results: people first, then meetups, then venues
    if (type === 'all') {
      const ordered: {
        users?: UserResult[];
        meetups?: MeetupResult[];
