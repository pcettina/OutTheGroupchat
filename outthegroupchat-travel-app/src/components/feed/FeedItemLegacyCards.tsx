...
export function TripCard({ trip }: { trip: TripPayload }) {
  const safeTripId = sanitizeRouteSegment(trip.id);
  const safeTripCoverImage = sanitizeUrl(trip.coverImage);
  const safeTripTitle = sanitizeText(trip.title);
  const safeTripCity = sanitizeText(trip.destination.city);
  const safeTripCountry = sanitizeText(trip.destination.country);
...
export function ActivityCard({ activity }: { activity: ActivityPayload }) {
  const safeActivityName = sanitizeText(activity.name);
  const safeActivityDescription = sanitizeText(activity.description);
