...
5. E2E Playwright tests for new critical paths: signup → Crew request → meetup create → RSVP → check-in — **🟢 COMPLETE 2026-06-11 (nightly/2026-06-11): `e2e/authenticated-flow.spec.ts` (16 tests, Crew→Meetup loop) now PASSES 16/16 in a real Chromium browser.** Production behavior was already correct; spec assertions were corrected to match the intentional middleware 307-redirects for gated API routes (`/api/meetups`, `/api/checkins/*`, `/api/notifications/*`) and a real signed NextAuth JWT cookie (helper `e2e/auth-helper.ts`) for authed-UI tests. No production code changed. **CI now guards these authenticated flows (2026-06-12):** `.github/workflows/ci.yml` runs `npm run build` before the Playwright step so the production `webServer` (`npm run start`) has a `.next` build to serve — the suite runs on every PR (closes prior-night recommendation #4).
   - *Progress (PR (nightly/2026-06-10)):* housekeeping pass only — dead-code cleanup (`src/lib/email-crew.ts`, `src/components/feed/ReactionPicker.tsx` removed), stale-doc content refresh (UPGRADE_PLAN, FUTURE_IMPLEMENTATION, IMPLEMENTATION_STACK, TEST_CASES reconciled to meetup-centric post-pivot reality), and `package.json` brand-metadata fix.
...
> **🟢 Phase 8 code-side work is COMPLETE as of 2026-06-11; CI now guards the authenticated flows as of 2026-06-12.** All actions that require code (action #1 checklist rewrite, #2 roadmap, #3 security audit incl. check-in stalking mitigation, #4 rate-limit audit, #5 authenticated-flow E2E now passing 16/16 in a real browser **and wired into CI behind a production build (2026-06-12)**, #6 Sentry coverage ~63/64 non-archive routes) are landed. The **only remaining Phase 8 items are operational/infrastructure tasks that require Vercel config, not code**: Sentry DSN in Vercel prod, Pusher env vars, Resend domain verification, uptime monitor, and the NEXTAUTH_SECRET strength audit. Phase definitions and objectives are unchanged.

> *Last refactor-plan update: 2026-06-12 (nightly/2026-06-12) — CI now guards the authenticated flows: `.github/workflows/ci.yml` runs `npm run build` before the Playwright E2E step so the verified `e2e/authenticated-flow.spec.ts` suite (16/16 local) runs on every PR (closes prior-night recommendation #4). No phase definitions/objectives changed; Phase 8 code-side work remains complete, remaining gates are operational/infra-only. (Prior: 2026-06-11, nightly/2026-06-11 — action #5 authenticated-flow E2E marked ✅ COMPLETE, passing 16/16 in real Chromium.)*
