...
**Current stats (2026-07-23):** 70 live API routes (excluding `_archive`; 83 raw route.ts — +1, Day 12 added `profile/avatar`) | 118 vitest-active test files | 2372 tests passing | ~377 TS/TSX files (excluding `_archive`) | `any`: 0 | `console.*`: 0 | TODO/FIXME: 0 | prod files >600 lines: 0 | tsc: 0 errors | lint: 0/0 | prisma: valid. Day 12 of BUILD_PLAN.md shipped Profile + auth depth: NEW `POST/DELETE /api/profile/avatar` sets/clears `User.image`, backed by NEW dependency-free `src/lib/storage.ts` (Vercel Blob wrapper; magic-byte content-type sniffing; missing storage env → HTTP 503 `STORAGE_NOT_CONFIGURED`, never a crash), with UI wired via NEW `src/app/settings/profile/`; `src/lib/auth.ts` gained explicit `session.maxAge`=30d + `session.updateAge`=24h + aligned `jwt.maxAge` and threads `image` through the JWT/session; failed-login backoff via NEW pure `src/lib/login-backoff.ts` wired into the credentials `authorize()`. SECURITY FIX: `GET /api/users/[userId]` now enforces `getServerSession()` 401 (was unauthenticated enumeration) with the client-writable `preferences` blob removed. NEW test files `src/__tests__/lib/storage.test.ts` (47), `src/__tests__/api/avatar.test.ts` (25), `src/__tests__/lib/login-backoff.test.ts` (48), `src/__tests__/lib/auth-session.test.ts` (23); `src/__tests__/api/users.test.ts` 15→21. No `schema.prisma`/`setup.ts` changes. New env dependency: `BLOB_READ_WRITE_TOKEN` in Vercel prod. PR https://github.com/pcettina/OutTheGroupchat/pull/152.

**Previous stats (2026-07-22, Day 11):** 69 live API routes (excluding `_archive`; 82 raw route.ts — +1, Day 11 added `meetups/[id]/ics`) | 114 vitest-active test files | 2223 tests passing | ~375 TS/TSX files (excluding `_archive`) | `any`: 0 | `console.*`: 0 | TODO/FIXME: 0 | prod files >600 lines: 0 | tsc: 0 errors | lint: 0/0 | prisma: valid. Day 11 of BUILD_PLAN.md rounded out the durable Meetup surface: NEW `GET /api/meetups/[id]/ics` (RFC 5545 `VCALENDAR`, `text/calendar` attachment, time + venue); `PATCH`/`DELETE /api/meetups/[id]` now fire fail-soft attendee notifications (`type: SYSTEM` + `data.kind` `MEETUP_UPDATED`/`MEETUP_CANCELLED`) on edit/cancel, with NEW `src/components/meetups/EditMeetupModal.tsx` + "Add to calendar" anchor on `src/app/meetups/[id]/page.tsx`; `POST /api/feed/comments` parses `@handle` mentions via NEW pure `src/lib/mentions.ts` (mention notification per mentioned user) with mention links rendered in `src/components/feed/CommentThread.tsx`. NEW test files `src/__tests__/lib/mentions.test.ts` (18), `src/__tests__/api/feed-comments-mentions.test.ts` (5), `src/__tests__/api/meetup-ics.test.ts` (19), `src/__tests__/api/meetup-notify.test.ts` (8). No `schema.prisma`/`setup.ts` changes. PR https://github.com/pcettina/OutTheGroupchat/pull/151.
...
|------|---------|---------|
| `lib/auth.ts` | authOptions, getServerSession, SESSION_MAX_AGE_SECONDS, SESSION_UPDATE_AGE_SECONDS | NextAuth config (Google + Credentials); explicit `session.maxAge`=30d + `session.updateAge`=24h + aligned `jwt.maxAge`; threads `image` through `authorize()`→`jwt`→`session`; credentials `authorize()` applies `login-backoff` lockout BEFORE the DB lookup (BUILD_PLAN Day 12) |
| `lib/login-backoff.ts` | checkLoginBackoff, registerFailedLogin, clearLoginBackoff (+ pure reducers: recordFailure, isLocked, normalizeLoginKey) | Failed-login backoff — pure reducers over a bounded in-memory store, injectable clock (`MAX_FAILED_ATTEMPTS`=5, `LOCKOUT_MS`=15m); NOT built on `checkRateLimit` (which fails open when Upstash env is unset) (BUILD_PLAN Day 12) |
| `lib/storage.ts` | putAvatar, isStorageConfigured, sniffImageType, isAllowedAvatarType (`AVATAR_MAX_BYTES`, `ALLOWED_AVATAR_TYPES`) | Dependency-free provider-agnostic Vercel Blob wrapper; `sniffImageType` magic-byte sniffing is the authoritative content-type check; `putAvatar` never throws (returns a `StorageResult`); `deps.fetchImpl` injection seam; unconfigured storage → callers return HTTP 503 `STORAGE_NOT_CONFIGURED` (BUILD_PLAN Day 12) |
...
**Total: 118 Vitest unit/integration test files, 2372 tests passing** (nightly/2026-07-23, Day 12 "Profile + auth depth"; new `src/__tests__/lib/storage.test.ts` (47) + `src/__tests__/api/avatar.test.ts` (25) + `src/__tests__/lib/login-backoff.test.ts` (48) + `src/__tests__/lib/auth-session.test.ts` (23); `src/__tests__/api/users.test.ts` 15→21 (5 tests encoding the old insecure 200 contract corrected + 6 invariant tests); no new `setup.ts` mocks; 0 TSC errors). Previous: 114 files / 2223 tests (nightly/2026-07-22, Day 11 "Meetup depth: edit/cancel + .ics + @mentions"). Separately, `e2e/` adds 16 Playwright authenticated-flow tests (run via `npm run test:e2e`) — **passing 16/16 in a real Chromium browser**, not counted in the Vitest suite.

| File | Lines | Tests | Coverage |
|------|-------|-------|----------|
| `src/__tests__/lib/storage.test.ts` | — | 47 | `src/lib/storage.ts` — magic-byte content-type sniffing (authoritative check), size/type validation, missing-storage-env path, `deps.fetchImpl` injection seam, `putAvatar` never-throws contract ✅ 2026-07-23 nightly/2026-07-23 (BUILD_PLAN Day 12) |
| `src/__tests__/api/avatar.test.ts` | — | 25 | `POST/DELETE /api/profile/avatar` — auth 401, HTTP 503 `STORAGE_NOT_CONFIGURED` on missing env (never a crash), `User.image` set on POST / cleared on DELETE, Zod + rate-limit paths ✅ 2026-07-23 nightly/2026-07-23 (BUILD_PLAN Day 12) |
| `src/__tests__/lib/login-backoff.test.ts` | — | 48 | `src/lib/login-backoff.ts` — pure threshold/lockout/reset reducers, bounded in-memory store, injectable-clock expiry ✅ 2026-07-23 nightly/2026-07-23 (BUILD_PLAN Day 12) |
| `src/__tests__/lib/auth-session.test.ts` | — | 23 | `src/lib/auth.ts` — explicit `session.maxAge`/`updateAge`/`jwt.maxAge` config assertions + credentials lockout ordering (lockout check runs BEFORE the `prisma.user.findUnique` lookup + bcrypt compare) ✅ 2026-07-23 nightly/2026-07-23 (BUILD_PLAN Day 12) |
...
| `src/__tests__/api/users.test.ts` | — | 21 | Users API — `GET /api/users/[userId]` now asserts the **secure contract** (401 when unauthenticated; no `preferences` blob leaked) after the 2026-07-23 security fix (5 tests that encoded the old insecure 200 contract corrected + 6 invariant tests added) ✅ 2026-07-23 nightly/2026-07-23 (BUILD_PLAN Day 12) |
...
| Vitest tests | 2372 passing, 118 test files (nightly/2026-07-23, BUILD_PLAN Day 12; +47 `storage.test.ts` +25 `avatar.test.ts` +48 `login-backoff.test.ts` +23 `auth-session.test.ts`; `users.test.ts` 15→21); archived tests runnable on demand via `npm run test:archive` |
...
| Live API routes | 70 (excluding `_archive`; 83 raw route.ts) as of 2026-07-23: 35 base + 6 Crew + 9 Phase 4 meetup/venue/cron + 3 Phase 5 check-in + privacy + 14 V1 routes (intents 4 + subcrews 6 + topics + heatmap + recommendations + cron/expire-intents) + `users/relationship-settings` (Day 4) + `users/[userId]/block` (Day 5) + `reports` (Day 6) + `users/onboarding` (Day 7) + `heatmap/contributor-count` (Day 8) + `crew/suggestions` + `checkins/ping` (Day 10) + `meetups/[id]/ics` (Day 11) + `profile/avatar` (Day 12); feed POST now 410; AI routes deleted 2026-04-23 |
| TS/TSX files | ~377 (`find src` excl `_archive`/node_modules/.next) as of 2026-07-23 |
...
| Production env gaps | Pusher vars, Sentry DSN, Resend domain, GOOGLE_PLACES_API_KEY, **`BLOB_READ_WRITE_TOKEN` (avatar upload returns 503 until set — BUILD_PLAN Day 12)** |
