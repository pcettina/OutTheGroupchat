...
> Auto-generated 2026-03-10. Last updated 2026-04-22 (Phase 7 in progress — About page, OG metadata, README rewrite, email-auth.ts, FeedItem sub-components, ProfileCheckinsSection, AI route test suites). Comprehensive reference for agents and developers.
>
...
**Stats (Phase 7 in progress, 2026-04-22):** 52 live API routes (35 base + 6 Crew routes + 9 Phase 4 meetup/venue/cron routes + 3 Phase 5 check-in routes + privacy route + 2 Phase 6 AI routes: suggest-meetups, icebreakers; 13 archived in Phase 1; feed POST now 410) | live component groups: auth, feed (rescoped to meetup/checkin types, tabs updated; `FeedItemHeader`, `FeedItemActions`, `FeedItemLegacyCards`, `FeedItemTypes` extracted), social (incl. `CrewButton`, `CrewRequestCard`, `CrewList`), meetups (incl. `MeetupCard`, `MeetupList`, `CreateMeetupModal`, `RSVPButton`, `VenuePicker`, `AttendeeList`, `MeetupInviteModal`), checkins (incl. `CheckInButton`, `LiveActivityCard`, `NearbyCrewList`), discover, notifications, profile (incl. `ProfileCheckinsSection`), search, settings (incl. `PrivacySettingsForm`), onboarding, ai, ui, accessibility + Navigation (incl. privacy link) | live pages: /, /about, /auth/*, /profile, `/profile/[userId]`, /feed, /discover, /inspiration, /notifications, /search, /settings, `/settings/privacy`, /onboarding, /privacy, /terms, `/crew`, `/crew/requests`, `/meetups`, `/meetups/new`, `/meetups/[id]`, `/checkins`, `/checkins/[id]` | middleware: auth-protects `/profile/:path*`, `/crew/:path*`, `/meetups/:path*`, `/checkins/:path*`, `/settings/:path*`, `/api/checkins/*`, plus select `/api/*` paths
**Test Health (2026-04-22, nightly/2026-04-23):** 58 live test files | ~1101 tests passing | 0 TSC errors | Phase 7: About page ✅, OG metadata ✅, README rewrite ✅, email-auth.ts ✅, FeedItem sub-components ✅, ProfileCheckinsSection ✅, AI route test suites ✅ (30+23); favicon deferred
