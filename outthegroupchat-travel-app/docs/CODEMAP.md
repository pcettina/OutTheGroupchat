...
> Auto-generated 2026-03-10. Comprehensive reference for agents and developers.
...
**Stats:** ~196 TS/TSX files | ~33,000 LOC | 47 API routes | 94 components | 20 pages
...
│   │   ├── profile/page.tsx
│   │   └── api/                   # 47 API route files (see API Routes section)
...
**Utility pages:** `error.tsx` (error boundary), `not-found.tsx` (404), `loading.tsx` (skeleton)
...
| File | Lines | Coverage |
|------|-------|----------|
| `src/__tests__/api/trips.test.ts` | 525 | 30 Vitest tests — trips API (GET, POST, PATCH, DELETE) |
| `src/__tests__/setup.ts` | — | Test environment/fixture config |
| `vitest.config.ts` | 16 | Vitest runner configuration |

**Gap:** Only trips API is tested. No component tests, no e2e (Playwright), no service tests.
...
|--------|--------|
| Lint warnings | 0 |
| `any` types | ~18 remaining |
| `console.*` | ~75 remaining |
| Test coverage | Trips API only (30 tests) |
| Files >400 lines | 12 (consider splitting) |
| Monitoring | None (Sentry not set up) |
| Production env gaps | OPENAI_API_KEY, Pusher vars, Resend domain |
