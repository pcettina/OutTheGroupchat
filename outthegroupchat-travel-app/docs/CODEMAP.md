...
**Total: 105 Vitest unit/integration test files, 2032 tests passing** (nightly/2026-07-20, Day 8 "Tighten the notification loop"; new `src/__tests__/api/heatmap-contributor-count.test.ts` (24) + `src/__tests__/components/notification-preferences-form.test.ts` (20), extended `api/notification-preferences.test.ts` (+13) and `lib/daily-prompt.test.ts` (+1); 0 TSC errors). Separately, `e2e/` adds 16 Playwright authenticated-flow tests (run via `npm run test:e2e`) — **passing 16/16 in a real Chromium browser**, not counted in the Vitest suite.
