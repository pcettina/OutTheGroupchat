...
**Total: 100 Vitest unit/integration test files, 1946 tests passing** (nightly/2026-07-17, Day 6 "Trust & Safety II: report + anti-spam"; new `src/__tests__/api/reports.test.ts` (15) + daily-quota 429 (2); 0 TSC errors). Separately, `e2e/` adds 16 Playwright authenticated-flow tests (run via `npm run test:e2e`) — **passing 16/16 in a real Chromium browser**, not counted in the Vitest suite.
...
| Vitest tests | 1946 passing, 100 test files (nightly/2026-07-17, BUILD_PLAN Day 6; +15 `reports.test.ts` +2 daily-quota 429); archived tests runnable on demand via `npm run test:archive` |
...
| Live API routes | 64 (excluding `_archive`; 77 raw route.ts) as of 2026-07-17: 35 base + 6 Crew + 9 Phase 4 meetup/venue/cron + 3 Phase 5 check-in + privacy + 14 V1 routes (intents 4 + subcrews 6 + topics + heatmap + recommendations + cron/expire-intents) + `users/relationship-settings` (Day 4) + `users/[userId]/block` (Day 5) + `reports` (Day 6); feed POST now 410; AI routes deleted 2026-04-23 |
| TS/TSX files | ~426 (`find src` excl node_modules/.next) as of 2026-07-17 |
