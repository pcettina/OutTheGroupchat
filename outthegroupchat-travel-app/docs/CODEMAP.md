...
**Current stats (2026-06-29):** 61 live API routes (excluding `_archive`) | 93 vitest-active test files | 1861 tests passing | ~335 active (non-archive) TS/TSX files | `any`: 0 | `console.*`: 0 | TODO/FIXME: 0 | files >600 lines (active): 0 (largest 513) | tsc: 0 errors | lint: 0/0 | prisma: valid | Sentry coverage ~63/64 non-archive routes | E2E `authenticated-flow.spec.ts` passing 16/16 in real Chromium, wired into CI behind a production build. Day 1 of BUILD_PLAN.md shipped the real hotness signal (`computeHotnessBoost` + recommendations ranking wiring).
...
**Codebase Health metrics (2026-06-29):** `any` types: 0 | `console.*`: 0 | TODO/FIXME: 0 | files >600 lines (active): 0 (largest 513) | API routes: 61 | test files: 93 | tests: 1861 | active (non-archive) TS/TSX files: ~335 | CI builds before Playwright E2E (`.github/workflows/ci.yml`) so the authenticated-flow suite runs on every PR
...
**Total: 1861 tests across 93 Vitest unit/integration test files** (nightly/2026-06-30, Day 1 "Hotness signal goes live"; new `src/__tests__/lib/hotness-score.test.ts` (16) + recommendations boost-reorder tests (+3) + repaired recommendations-edge / v1-misc fixtures; 0 TSC errors). Separately, `e2e/` adds 16 Playwright authenticated-flow tests (run via `npm run test:e2e`) — **passing 16/16 in a real Chromium browser**, not counted in the 1861.
...
| TS/TSX files | 334 active (non-archive) as of 2026-06-12 (prior docs showed 333 — 1-file count drift corrected against the live tree) |
