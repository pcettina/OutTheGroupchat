...
**Current stats (2026-07-16):** 63 live API routes (excluding `_archive`; 76 raw route.ts) | 121 vitest-active test files | 1929 tests passing | 424 TS/TSX files | `any`: 0 | `console.*`: 0 | TODO/FIXME: 0 | prod files >600 lines: 0 | tsc: 0 errors | lint: 0/0 | prisma: valid | Sentry coverage ~63/64 non-archive routes | E2E `authenticated-flow.spec.ts` passing 16/16 in real Chromium, wired into CI behind a production build. Day 5 of BUILD_PLAN.md shipped baseline Trust & Safety: NEW `prisma.UserBlock` model + `POST/DELETE /api/users/[userId]/block` (idempotent; auto-severs the Crew edge on block), mutual block enforcement across crew/feed/heatmap/check-in-feed (no partial leak), NEW UI `BlockButton.tsx` on `/profile/[userId]` + `/crew`. (`next build` compiles OK; the Next 14.2.35 type-check worker env-crashes on this Windows host — env flake, not a code defect.)
...
**Total: 121 Vitest unit/integration test files, 1929 tests passing** (nightly/2026-07-16, Day 5 "Trust & Safety I: block a user"; new `src/__tests__/api/user-block.test.ts` (12) + `src/__tests__/api/block-enforcement.test.ts` (8); 0 TSC errors). Separately, `e2e/` adds 16 Playwright authenticated-flow tests (run via `npm run test:e2e`) — **passing 16/16 in a real Chromium browser**, not counted in the Vitest suite.
...
| Vitest tests | 1929 passing, 121 test files (nightly/2026-07-16, BUILD_PLAN Day 5; +12 `user-block.test.ts` +8 `block-enforcement.test.ts`); archived tests runnable on demand via `npm run test:archive` |
...
| Live API routes | 63 (excluding `_archive`; 76 raw route.ts) as of 2026-07-16: 35 base + 6 Crew + 9 Phase 4 meetup/venue/cron + 3 Phase 5 check-in + privacy + 14 V1 routes (intents 4 + subcrews 6 + topics + heatmap + recommendations + cron/expire-intents) + `users/relationship-settings` (Day 4) + `users/[userId]/block` (Day 5); feed POST now 410; AI routes deleted 2026-04-23 |
| TS/TSX files | 424 (`find src` excl node_modules/.next) as of 2026-07-16 |
