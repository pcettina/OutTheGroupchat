...
**Total: 103 Vitest unit/integration test files, 1976 tests passing** (nightly/2026-07-19, Day 7 "Activation: real onboarding + empty/error states"; new `src/__tests__/api/onboarding.test.ts` (13) + `src/__tests__/onboarding-flow.test.ts` (12) + `src/__tests__/components/empty-error-states.test.ts` (5); 0 TSC errors). Separately, `e2e/` adds 16 Playwright authenticated-flow tests (run via `npm run test:e2e`) — **passing 16/16 in a real Chromium browser**, not counted in the Vitest suite.
...
| Vitest tests | 1976 passing, 103 test files (nightly/2026-07-19, BUILD_PLAN Day 7; +13 `onboarding.test.ts` +12 `onboarding-flow.test.ts` +5 `empty-error-states.test.ts`); archived tests runnable on demand via `npm run test:archive` |
...
| Live API routes | 65 (excluding `_archive`; 78 raw route.ts) as of 2026-07-19: 35 base + 6 Crew + 9 Phase 4 meetup/venue/cron + 3 Phase 5 check-in + privacy + 14 V1 routes (intents 4 + subcrews 6 + topics + heatmap + recommendations + cron/expire-intents) + `users/relationship-settings` (Day 4) + `users/[userId]/block` (Day 5) + `reports` (Day 6) + `users/onboarding` (Day 7); feed POST now 410; AI routes deleted 2026-04-23 |
| TS/TSX files | ~429 (`find src` excl node_modules/.next) as of 2026-07-19 |
