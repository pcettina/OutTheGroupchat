...
> **Last Updated: 2026-07-23** (nightly/2026-07-23 — BUILD_PLAN.md Day 12 "Profile + auth depth": **+1 live route → 70 live routes** (excluding `_archive`; 83 raw route.ts). NEW `POST/DELETE /api/profile/avatar` (`src/app/api/profile/avatar/route.ts`) — POST accepts an image upload and sets `User.image`, DELETE clears it; both are auth-gated (`getServerSession()` 401) + Zod + rate-limited + Sentry. Backed by NEW dependency-free `src/lib/storage.ts` (provider-agnostic Vercel Blob wrapper; **magic-byte sniffing is the authoritative content-type check**, `putAvatar` never throws, `deps.fetchImpl` injection seam). When the storage env is missing the route returns **HTTP 503 `STORAGE_NOT_CONFIGURED`** — graceful degrade, never a crash (the plan's headline acceptance criterion). `next.config.js` `images.remotePatterns` gained `**.public.blob.vercel-storage.com`. **SECURITY FIX:** `GET /api/users/[userId]` called `getServerSession` but never enforced it — added the 401 gate (was unauthenticated enumeration of any user's name/image/bio/city/crewLabel/crewCount) and removed the free-form client-writable `preferences` JSON blob from the route entirely (self-view preferences remain served by the 401-gated `/api/users/me`). `MODIFIED src/app/api/profile/route.ts` — fixed a real bug where `image` was absent from BOTH the GET and PUT selects (avatars never round-tripped) and tightened the PUT Zod schema (`.url()`, name ≤100, bio ≤500, city ≤100). **Session hardening (no route change):** `src/lib/auth.ts` sets explicit `session.maxAge`=30d + `session.updateAge`=24h + aligned `jwt.maxAge` (exported `SESSION_MAX_AGE_SECONDS`/`SESSION_UPDATE_AGE_SECONDS`), and threads `image` through `authorize()` → `jwt` → `session`. **Failed-login backoff:** NEW `src/lib/login-backoff.ts` (pure reducers + bounded in-memory store, injectable clock) wired into the credentials `authorize()` — the lockout check runs BEFORE the DB lookup + bcrypt compare, `registerFailedLogin` on both failure branches, `clearLoginBackoff` on success (deliberately NOT built on `checkRateLimit`, which fails open when Upstash env is unset). NEW UI mount `src/app/settings/profile/page.tsx` + `ProfileSettingsForm.tsx` (`ProfileSettings.tsx` was previously mounted by no page — wired file input + remove button). No `schema.prisma` or `setup.ts` changes were needed (only `prisma.user` delegates used; all mocks present). Tests → **118 files / 2372 tests** (+143 net: `lib/storage.test.ts` (47), `api/avatar.test.ts` (25), `lib/login-backoff.test.ts` (48), `lib/auth-session.test.ts` (23), and `api/users.test.ts` 15→21 where 5 tests encoding the old insecure 200 contract were corrected + 6 invariant tests added). tsc 0, lint 0/0, full vitest 2372/2372, prisma valid. **New env dependency:** `BLOB_READ_WRITE_TOKEN` must be set in Vercel prod or avatar upload returns 503. PR https://github.com/pcettina/OutTheGroupchat/pull/152.)
> **Previous (2026-07-22, nightly/2026-07-22 — BUILD_PLAN.md Day 11 "Meetup depth: edit/cancel + .ics + @mentions": **+1 live route → 69 live routes** (excluding `_archive`; 82 raw route.ts). NEW `GET /api/meetups/[id]/ics` (`src/app/api/meetups/[id]/ics/route.ts`) — returns an RFC 5545 `VCALENDAR` (`text/calendar` attachment) with time + venue, powering the "Add to calendar" button on meetup detail. Two existing routes gained additive enhancements: `PATCH`/`DELETE /api/meetups/[id]` (`src/app/api/meetups/[id]/route.ts`) now fire fail-soft attendee notifications (`type: SYSTEM` + `data.kind` `MEETUP_UPDATED`/`MEETUP_CANCELLED`) on edit/cancel, and `POST /api/feed/comments` (`src/app/api/feed/comments/route.ts`) now parses `@handle` mentions via NEW `src/lib/mentions.ts` (pure helpers) and fires a mention notification per mentioned user, with mention links rendered in `src/components/feed/CommentThread.tsx`. NEW UI `src/components/meetups/EditMeetupModal.tsx` mounted (Edit button + "Add to calendar" anchor) on `src/app/meetups/[id]/page.tsx`. No `schema.prisma` or `setup.ts` changes were needed (all delegates already present). Tests → **114 files / 2223 tests** (+50: `lib/mentions.test.ts` (18) + `api/feed-comments-mentions.test.ts` (5) + `api/meetup-ics.test.ts` (19) + `api/meetup-notify.test.ts` (8)). tsc 0, lint 0/0, full vitest 2223/2223, prisma valid. PR https://github.com/pcettina/OutTheGroupchat/pull/151.)
...
>
> **Live API routes (post-V1):** 59 at the time of the V1 pivot — **now 70 (excluding `_archive`; 83 raw route.ts) as of 2026-07-23**
>
...
> **Live API routes (current):** **70 live** (excluding `_archive`; 83 raw route.ts) + 13 archived. Active surface: 35 base routes + 6 Crew + 9 Phase 4 meetup/venue/cron + 3 Phase 5 check-in + privacy + 14 V1 routes (intents/subcrews/topics/heatmap/recommendations/cron-expire-intents) + `users/relationship-settings` (Day 4) + `users/[userId]/block` (Day 5) + `reports` (Day 6) + `users/onboarding` (Day 7) + `heatmap/contributor-count` (Day 8) + `crew/suggestions` + `checkins/ping` (Day 10) + `meetups/[id]/ics` (Day 11) + `profile/avatar` (Day 12). Feed POST now returns 410. (Historical: 72 total / 59 active at the Phase 1 archive baseline.)
...
| `/api/feed/comments` | POST | ✅ | ✅ | **Sentry added 2026-04-16**; **@mentions 2026-07-22 (BUILD_PLAN Day 11, PR #151)** — parses `@handle` via new `src/lib/mentions.ts` (pure helpers) and fires a mention notification per mentioned user; `CommentThread.tsx` renders mentions as links |
...
| Endpoint | Method | Status | Frontend Connected | Notes |
|----------|--------|--------|-------------------|-------|
| `/api/profile` | GET | ✅ | ✅ | Get current user; **2026-07-23 (BUILD_PLAN Day 12): bug fix — `image` added to the select so avatars round-trip** |
| `/api/profile` | PUT | ✅ | ✅ | Update profile; Zod validation added 2026-03-13; **2026-07-23 (Day 12): `image` added to select + PUT Zod tightened (`.url()`, name ≤100, bio ≤500, city ≤100)** |
| `/api/profile/avatar` | POST | ✅ | ✅ | **Implemented 2026-07-23 (BUILD_PLAN Day 12, Profile + auth depth).** Uploads an avatar image and sets `User.image`. Auth (`getServerSession()` 401) + Zod + rate-limit + Sentry. Backed by `src/lib/storage.ts` (Vercel Blob wrapper; magic-byte content-type sniffing). **Missing storage env → HTTP 503 `STORAGE_NOT_CONFIGURED`, never a crash.** Requires `BLOB_READ_WRITE_TOKEN` in prod. UI: `src/app/settings/profile/` |
| `/api/profile/avatar` | DELETE | ✅ | ✅ | **Implemented 2026-07-23 (BUILD_PLAN Day 12).** Clears `User.image`. Auth-gated; idempotent |
| `/api/users/[userId]` | GET | ✅ | ✅ | Public user profile — returns `crewCount`, `crewLabel`; `isFollowing` and `publicTrips` removed in Phase 3 Part B; wired to `/profile/[userId]` page. **SECURITY FIX 2026-07-23 (BUILD_PLAN Day 12): now enforces `getServerSession()` 401** (previously called it but never enforced it → unauthenticated enumeration) and the free-form client-writable `preferences` JSON blob was removed from the route (self-view preferences remain served by the 401-gated `/api/users/me`) |
...
| `/api/meetups/[id]` | PATCH | ✅ | Edit meetup (host only); broadcasts `meetup:updated` on Pusher `meetup-{id}` channel (Session 2, 2026-04-18); **fires fail-soft attendee notifications (`type: SYSTEM`, `data.kind: MEETUP_UPDATED`) on edit (2026-07-22, BUILD_PLAN Day 11, PR #151)** |
| `/api/meetups/[id]` | DELETE | ✅ | Cancel meetup; broadcasts `meetup:cancelled` (Session 2, 2026-04-18); **fires fail-soft attendee notifications (`type: SYSTEM`, `data.kind: MEETUP_CANCELLED`) on cancel (2026-07-22, BUILD_PLAN Day 11, PR #151)** |
| `/api/meetups/[id]/ics` | GET | ✅ | **Implemented 2026-07-22 (BUILD_PLAN Day 11, PR #151).** Returns an RFC 5545 `VCALENDAR` (`text/calendar` attachment) for the meetup — time + venue — powering the "Add to calendar" button on meetup detail. Backed by `src/app/api/meetups/[id]/ics/route.ts` |
