...
*Last Updated: 2026-07-11 (nightly/2026-07-11, BUILD_PLAN Day 3) — no route additions/removals (74 routes on current audit). `PATCH /api/subcrews/[id]` now graduates a frozen SubCrew (`startAt`+`venueId` set) into a CREW Meetup: creates one Meetup (`scheduledAt=startAt`, `endsAt=endAt`), links all `SubCrewMember`s as `MeetupAttendee`s, sets `SubCrew.meetupId`; idempotent (transaction + `updateMany` claim + `@unique`); response gains `meetup`+`graduated`. New lib `src/lib/subcrews/graduate-to-meetup.ts` + `src/__tests__/api/subcrew-graduation.test.ts` (13). Tests: 118 files. tsc 0, lint 0/0, build PASS, prisma valid. PR pending.*
