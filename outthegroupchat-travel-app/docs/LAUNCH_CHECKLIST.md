...
- [~] Post-pivot steady state (V1 product vision): intent → group → heatmap surface live. **V1 Phase 5 (opt-in notifications) COMPLETE 2026-06-02** — `/api/users/notification-preferences` (GET/PATCH), `/api/cron/send-daily-prompts` cron, `/settings/notifications` opt-in UI, PER_MEMBER_INTENT trigger (`src/lib/notifications/per-member-intent.ts` wired into `/api/intents`), GROUP_FORMATION push (`src/lib/subcrew/try-form.ts`) all wired ✅
...
- [x] PER_MEMBER_INTENT trigger wired ✅ 2026-06-02 — `src/lib/notifications/per-member-intent.ts` dispatches a SYSTEM notification to opted-in ACCEPTED-Crew watchers on Intent.create (wired into `/api/intents`)
- [x] GROUP_FORMATION push wired ✅ 2026-06-02 — `src/lib/subcrew/try-form.ts` broadcasts `subcrew:formed` via Pusher to seed users with GROUP_FORMATION enabled (additive, non-fatal)
...
*Last Updated: 2026-06-02 — V1 Phase 5 (opt-in notifications) COMPLETE: `GET/PATCH /api/users/notification-preferences`, `GET /api/cron/send-daily-prompts` cron (DAILY `0 13 * * *`), `/settings/notifications` opt-in UI, PER_MEMBER_INTENT trigger + GROUP_FORMATION push wired; 1172 tests across 71 files; 74 route files. Post-pivot steady state executing the V1 product vision (intent → group → heatmap).*
